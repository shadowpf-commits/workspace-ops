...
// ─── Color Palette: Gaming / Esports Style ─────────────────────────────────────────
const C = {
  ...
  darkBg: "0F0A1A",        // Deep purple-black background
  cardBg: "1A1025",        // Card background
  cardElevated: "251A35",  // Elevated surface
  slate: "3D2952",         // Slate purple
...
async function buildPresentation() {
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // SLIDE 1: Title Slide - Gaming Style
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0, y: 0, w: 10, h: 5.625, fill: { color: C.purple, transparency: 85 }
    });
    
    // Top accent bar
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0, y: 0, w: 10, h: 0.1, fill: { color: C.purple }
    });
    
    // Left decorative bar
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0.5, y: 1.5, w: 0.08, h: 2.2, fill: { color: C.cyan }
    });
    
    // Glowing accent circle
    slide.addShape(pres.shapes.OVAL, {
      x: 0.7, y: 1.7, w: 0.6, h: 0.6, fill: { color: C.purple }
    });
    slide.addShape(pres.shapes.OVAL, {
      x: 0.85, y: 1.85, w: 0.3, h: 0.3, fill: { color: C.cyan }
    });
    
    // Main title with glow effect
    slide.addText("GOLD AND GLORY", {
      x: 1.5, y: 1.5, w: 8, h: 1.0,
      fontSize: 52, fontFace: "Arial Black", color: C.gold, charSpacing: 4
    });
    
    // Subtitle
    slide.addText("7月大版本完整发行策略", {
      x: 1.5, y: 2.5, w: 8, h: 0.7,
      fontSize: 28, fontFace: "Arial", color: C.text, bold: true
    });
    
    // Tagline with icons
    slide.addText("Arena  ·  PVE  ·  3C升级  ·  UA买量  ·  本地化", {
      x: 1.5, y: 3.3, w: 8, h: 0.5,
      fontSize: 16, fontFace: "Arial", color: C.purpleLight
    });
    
    // Divider
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 1.5, y: 4.0, w: 2.5, h: 0.03, fill: { color: C.purple }
    });
    
    // Date
    slide.addText("2026年4月  |  Confidential — Internal Use Only", {
      x: 1.5, y: 4.2, w: 8, h: 0.4,
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    // USP Quote box
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0.5, y: 1.1, w: 9, h: 1.0, fill: { color: C.cardBg }
    });
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0.5, y: 1.1, w: 0.08, h: 1.0, fill: { color: C.cyan }
    });
    ...
    // 4 cards - 2x2 grid
    const cards = [
      { title: "产品定位", content: "中世纪地牢 Extraction RPG\n5–15分钟快节奏单局\n赛季制 + 英雄收集", x: 0.5, y: 2.3, icon: "⚔", color: C.purple },
      { title: "核心特色", content: "Mobile 端独家 Extraction Shooter\n风险回报机制\n死后全失的刺激感", x: 5.05, y: 2.3, icon: "🛡", color: C.cyan },
      { title: "当前状态", content: "Google Play 评分：3.7 ⚠\n已测试：墨西哥、哥伦比亚\n服务器：4–6月停服维护", x: 0.5, y: 3.95, icon: "⚠", color: C.warning },
      { title: "7月目标", content: "7月大版本发布在即\n⚠ 3.7分是市场门槛\n目标：7月前改善至 3.8–4.0", x: 5.05, y: 3.95, icon: "🎯", color: C.success },
    ];
    ...
    for (const c of cards) {
      ...
      slide.addText(c.content, {
        ...
        fontSize: 13, fontFace: "Arial", color: C.text, lineSpacingMultiple: 1.3, margin: 0
      });
    }
  }
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    for (let i = 0; i < pillars.length; i++) {
      ...
      // Card
      slide.addShape(pres.shapes.RECTANGLE, {
        x: x, y: 1.5, w: 2.9, h: 3.7, fill: { color: C.cardBg }
      });
      ...
      slide.addText(p.rank, {
        x: x + 0.15, y: 1.7, w: 0.7, h: 0.5,
        fontSize: 28, fontFace: "Arial Black", color: p.color, bold: true, margin: 0
      });
      
      // Symbol
      slide.addShape(pres.shapes.OVAL, {
        x: x + 0.9, y: 2.25, w: 1.1, h: 1.1, fill: { color: p.color, transparency: 70 }
      });
      slide.addText(p.symbol, {
        x: x + 0.9, y: 2.25, w: 1.1, h: 1.1,
      ...
      slide.addText(p.title, {
        ...
        fontSize: 16, fontFace: "Arial", color: C.text, bold: true, align: "center", margin: 0
      });
      
      // Tags
      slide.addText(p.tags, {
        x: x + 0.1, y: 3.8, w: 2.7, h: 0.5,
        fontSize: 10, fontFace: "Arial", color: C.textMuted, align: "center", margin: 0
      });
      ...
      slide.addText(p.points.map((pt, idx) => ({
        ...
        x: x + 0.15, y: 4.3, w: 2.6, h: 0.65,
        fontSize: 10, fontFace: "Arial", color: C.text, margin: 0
      });
      
      // Badge
      slide.addShape(pres.shapes.RECTANGLE, {
        x: x + 0.35, y: 4.95, w: 2.2, h: 0.28, fill: { color: p.color, transparency: 80 }
      });
      slide.addText(p.badge, {
        x: x + 0.35, y: 4.95, w: 2.2, h: 0.28,
        fontSize: 10, fontFace: "Arial", color: p.color, bold: true, align: "center", valign: "middle", margin: 0
      });
    }
    ...
    slide.addText("三个内容支柱构成7月营销的核心素材库，所有广告素材、KOL合作均围绕这三个展开。", {
      ...
      fontSize: 11, fontFace: "Arial", color: C.textMuted, italic: true, align: "center"
    });
  }
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0.5, y: 1.05, w: 0.08, h: 0.7, fill: { color: C.gold }
    });
    slide.addText("核心USP：进去抢，活着出来就是你的（Extraction Shooter独家机制）", {
      x: 0.75, y: 1.05, w: 8.5, h: 0.7,
      fontSize: 16, fontFace: "Arial", color: C.gold, valign: "middle", margin: 0
    });
    ...
    for (let i = 0; i < pillars4.length; i++) {
      ...
      // Row card
      slide.addShape(pres.shapes.RECTANGLE, {
        x: 0.5, y: y, w: 9, h: 0.82, fill: { color: C.cardBg }
      });
      slide.addShape(pres.shapes.RECTANGLE, {
        x: 0.5, y: y, w: 0.06, h: 0.82, fill: { color: p.color }
      });
      
      // Number badge
      slide.addShape(pres.shapes.OVAL, {
        x: 0.72, y: y + 0.16, w: 0.5, h: 0.5, fill: { color: p.color }
      });
      slide.addText(p.num, {
        x: 0.72, y: y + 0.16, w: 0.5, h: 0.5,
        fontSize: 16, fontFace: "Arial", color: C.darkBg, bold: true, align: "center", valign: "middle", margin: 0
      });
      ...
      slide.addText("切入角度：" + p.angle, {
        x: 1.4, y: y + 0.42, w: 2.7, h: 0.35,
        fontSize: 10, fontFace: "Arial", color: C.text, margin: 0
      });
      
      // Users
      slide.addText("目标用户：" + p.users, {
        x: 4.2, y: y + 0.1, w: 2.4, h: 0.35,
        fontSize: 11, fontFace: "Arial", color: C.purpleLight, margin: 0
      });
      
      // Content
      slide.addText("内容：" + p.content, {
        x: 4.2, y: y + 0.42, w: 5.1, h: 0.35,
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    slide.addText("亚洲 · 拉美（北部/南部） · 巴西 — 三大市场，三套叙事", {
      ...
      fontSize: 12, fontFace: "Arial", color: C.purpleLight
    });
    ...
    for (let i = 0; i < regions.length; i++) {
      ...
      // Card
      slide.addShape(pres.shapes.RECTANGLE, {
        x: x, y: 1.4, w: 2.9, h: 3.85, fill: { color: C.cardBg }
      });
      slide.addShape(pres.shapes.RECTANGLE, {
        x: x, y: 1.4, w: 2.9, h: 0.6, fill: { color: C.cardElevated }
      });
      slide.addShape(pres.shapes.RECTANGLE, {
        x: x, y: 1.98, w: 2.9, h: 0.04, fill: { color: r.color }
      });
      
      // Flag & Name
      slide.addText(r.flag + " " + r.name, {
        x: x + 0.15, y: 1.45, w: 2.6, h: 0.5,
        fontSize: 13, fontFace: "Arial", color: C.text, bold: true, valign: "middle", margin: 0
      });
      ...
      for (let j = 0; j < rows.length; j++) {
        ...
        // Color dot
        slide.addShape(pres.shapes.OVAL, {
          x: x + 0.15, y: y + 0.08, w: 0.12, h: 0.12, fill: { color: r.color }
        });
        slide.addText(row.label, {
          x: x + 0.35, y: y, w: 0.9, h: 0.25,
          fontSize: 9, fontFace: "Arial", color: r.color, bold: true, margin: 0
        });
        slide.addText(row.value, {
          x: x + 0.15, y: y + 0.25, w: 2.6, h: 0.32,
          fontSize: 10, fontFace: "Arial", color: C.text, margin: 0
        });
      }
    }
    ...
    // Warning note
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0.5, y: 5.35, w: 9, h: 0.2, fill: { color: C.warning, transparency: 85 }
    });
    slide.addText("※ 非正式上线（测试阶段），预算有限。OM为主，KOL/KOC按区域单独评估", {
      x: 0.6, y: 5.35, w: 8.8, h: 0.2,
      fontSize: 9, fontFace: "Arial", color: C.warning, valign: "middle", margin: 0
    });
  }
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    for (let i = 0; i < headers.length; i++) {
      slide.addText(headers[i], {
        ...
        fontSize: 11, fontFace: "Arial", color: C.purpleLight, bold: true, align: "center", valign: "middle", margin: 0
      });
    }
    ...
    for (let i = 0; i < materials.length; i++) {
      ...
      for (let j = 0; j < vals.length; j++) {
        slide.addText(vals[j], {
          ...
          fontSize: 10, fontFace: "Arial", color: j === 0 ? C.cyan : C.text, 
    ...
    slide.addText("※ 预计素材总量：各市场约 100-200 个组合，优先测试 Arena + 组队类素材", {
      ...
      fontSize: 10, fontFace: "Arial", color: C.textMuted, italic: true
    });
  }
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    for (let i = 0; i < kolHeaders.length; i++) {
      slide.addText(kolHeaders[i], {
        ...
        fontSize: 10, fontFace: "Arial", color: C.purpleLight, bold: true, align: "center", valign: "middle", margin: 0
      });
    }
    ...
    for (let i = 0; i < kolTiers.length; i++) {
      ...
      slide.addShape(pres.shapes.RECTANGLE, {
        x: 0.5, y: y, w: 0.05, h: 0.7, fill: { color: tierColors[i] }
      });
      ...
      for (let j = 0; j < vals.length; j++) {
        slide.addText(vals[j], {
          ...
          fontSize: j === 0 ? 12 : 10, 
          fontFace: "Arial", 
          color: j === 0 ? tierColors[i] : (j === 5 ? C.purpleLight : C.text), 
          bold: j === 0,
          align: "center", valign: "middle", margin: 0
        });
      }
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    // Central hub
    slide.addShape(pres.shapes.OVAL, {
      x: 3.8, y: 1.6, w: 2.4, h: 2.4, fill: { color: C.purple }
    });
    slide.addShape(pres.shapes.OVAL, {
    ...
    slide.addText("全部目标市场\n7月同步上线", {
      ...
      fontSize: 14, fontFace: "Arial", color: C.purple, bold: true, align: "center", valign: "middle", margin: 0
    });
    ...
    for (const c of countries) {
      slide.addShape(pres.shapes.RECTANGLE, {
        x: c.x, y: c.y, w: 1.35, h: 0.65, fill: { color: C.cardBg }
      });
      slide.addShape(pres.shapes.RECTANGLE, {
        x: c.x, y: c.y, w: 0.05, h: 0.65, fill: { color: c.color }
      });
      slide.addText(c.flag + " " + c.name, {
        x: c.x + 0.1, y: c.y, w: 1.15, h: 0.65,
        fontSize: 11, fontFace: "Arial", color: C.text, align: "center", valign: "middle", margin: 0
      });
    }
    
    // Bottom highlight
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 2.5, y: 5.35, w: 5, h: 0.25, fill: { color: C.purple, transparency: 85 }
    });
    slide.addText("亚洲+拉美+美国同步发行，非分批上线", {
      x: 2.5, y: 5.35, w: 5, h: 0.25,
      fontSize: 12, fontFace: "Arial", color: C.purpleLight, bold: true, align: "center", valign: "middle", margin: 0
    });
  }
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    // Phase nodes
    const phases = [
      { month: "3月", name: "准备期", action: "评分+本地化", color: C.purpleLight },
      { month: "4–5月", name: "停服维护", action: "素材储备+KOL接触", color: C.purple },
      { month: "6月", name: "预热期", action: "官宣+预约开放", color: C.cyan },
      { month: "7月", name: "大版本爆发", action: "全力投放", color: C.pink },
    ...
    for (let i = 0; i < phases.length; i++) {
      ...
      // Node circle
      slide.addShape(pres.shapes.OVAL, {
        x: x + 0.55, y: 2.15, w: 0.5, h: 0.5, fill: { color: p.color }
      });
      
      // Month
      slide.addText(p.month, {
        x: x, y: 1.65, w: 1.6, h: 0.4,
        fontSize: 12, fontFace: "Arial", color: p.color, bold: true, align: "center", margin: 0
      });
    ...
    // Detail cards
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0.5, y: 3.5, w: 4.4, h: 1.95, fill: { color: C.cardBg }
    });
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0.5, y: 3.5, w: 0.06, h: 1.95, fill: { color: C.purple }
    });
    slide.addText("准备期 3月", {
      x: 0.7, y: 3.55, w: 4, h: 0.35,
      fontSize: 13, fontFace: "Arial", color: C.purple, bold: true, margin: 0
    });
    slide.addText([
      ...
      fontSize: 11, fontFace: "Arial", color: C.text, lineSpacingMultiple: 1.3, margin: 0
    });
    
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 5.1, y: 3.5, w: 4.4, h: 1.95, fill: { color: C.cardBg }
    });
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 5.1, y: 3.5, w: 0.06, h: 1.95, fill: { color: C.pink }
    });
    slide.addText("爆发期 7月", {
      x: 5.3, y: 3.55, w: 4, h: 0.35,
      fontSize: 13, fontFace: "Arial", color: C.pink, bold: true, margin: 0
    });
    slide.addText([
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    for (let i = 0; i < budgets.length; i++) {
      ...
      // Bar fill
      slide.addShape(pres.shapes.RECTANGLE, {
        x: 2.1, y: y + 0.12, w: barW, h: 0.32, fill: { color: b.color }
      });
      ...
      slide.addText(b.pct, {
        ...
        fontSize: 11, fontFace: "Arial", color: C.darkBg, bold: true, align: "center", valign: "middle", margin: 0
      });
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    for (let i = 0; i < priorities.length; i++) {
      ...
      // Card
      slide.addShape(pres.shapes.RECTANGLE, {
        x: 0.5, y: y, w: 9, h: 1.25, fill: { color: C.cardBg }
      });
      slide.addShape(pres.shapes.RECTANGLE, {
      ...
      // Label badge
      slide.addShape(pres.shapes.RECTANGLE, {
        x: 0.75, y: y + 0.12, w: 1.5, h: 0.32, fill: { color: p.color, transparency: 75 }
      });
      slide.addText(p.label, {
        x: 0.75, y: y + 0.12, w: 1.5, h: 0.32,
        fontSize: 11, fontFace: "Arial", color: p.color, bold: true, align: "center", valign: "middle", margin: 0
      });
      
      // Arrow
      slide.addText("→", {
        x: 2.35, y: y + 0.05, w: 0.3, h: 0.4,
        fontSize: 18, fontFace: "Arial", color: C.cyan, margin: 0
      });
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    slide.addText("下一步", {
      ...
      fontSize: 40, fontFace: "Arial Black", color: C.cyan, align: "center", charSpacing: 8, margin: 0
    });
    ...
    for (let i = 0; i < nextSteps.length; i++) {
      ...
      // Number circle
      slide.addShape(pres.shapes.OVAL, {
        x: 2.0, y: y, w: 0.45, h: 0.45, fill: { color: stepColors[i] }
      });
      ...
      slide.addText(nextSteps[i], {
        ...
        fontSize: 17, fontFace: "Arial", color: C.text, valign: "middle", margin: 0
      });
    }
    
    // Goal box
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 1.8, y: 4.6, w: 6.4, h: 0.6, fill: { color: C.cardBg }
    });
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 1.8, y: 4.6, w: 0.08, h: 0.6, fill: { color: C.pink }
    });
    slide.addText("🎯 目标：7月爆发期全力冲击下载 + 榜单", {
      x: 2.0, y: 4.6, w: 6.1, h: 0.6,
      fontSize: 16, fontFace: "Arial", color: C.pink, bold: true, valign: "middle", margin: 0
    });
  ...
  // ── Save ──────────────────────────────────────────────────────────────────
  await pres.writeFile({ fileName: "/Users/shadow/.openclaw/workspace/GoldAndGlory_七月发行策略_v5.pptx" });
  console.log("✅ Done! Saved to GoldAndGlory_七月发行策略_v5.pptx");
}
