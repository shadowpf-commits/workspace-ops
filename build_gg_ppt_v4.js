...
const C = {
  ...
  goldLight: "F5D78E",   
  goldDark: "B8860B",   
  midnight: "1A1A2E",     // Dark but not pure black
  cardBg: "252540",       // Lighter card background
  cardElevated: "2E2E4A", 
  slate: "3D3D5C",       
  text: "FFFFFF",         // Pure white for max contrast
  textDark: "F0F0F0",    // Slightly off-white
  textMuted: "A0A0B0",   // Lighter muted text
  accent: "FF6B35",      
  success: "3FB950",     
  warning: "FFD700",     
  danger: "FF6B6B",      
...
// ─── Shadow Factories (simplified for compatibility) ───────────────────────────
const makeShadow = () => ({
  type: "outer", blur: 4, offset: 2, angle: 135, color: "000000", opacity: 0.2
});

const makeCardShadow = () => ({
  type: "outer", blur: 3, offset: 1, angle: 135, color: "000000", opacity: 0.15
});
...
async function buildPresentation() {
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0, y: 0, w: 10, h: 0.15, fill: { color: C.gold }
    });
    
    // Left gold accent bar
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0.5, y: 1.5, w: 0.08, h: 2.5, fill: { color: C.gold }
    });
    
    // Main title
    slide.addText("GOLD AND GLORY", {
      x: 0.8, y: 1.5, w: 8.5, h: 1.0,
      fontSize: 54, fontFace: "Arial Black", color: C.gold, charSpacing: 4
    });
    
    // Subtitle
    slide.addText("7月大版本完整发行策略", {
      x: 0.8, y: 2.5, w: 8.5, h: 0.7,
    ...
    // Divider
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0.8, y: 4.0, w: 3, h: 0.03, fill: { color: C.gold }
    });
    ...
    // Bottom gold bar
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0, y: 5.475, w: 10, h: 0.15, fill: { color: C.gold }
    });
  }
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0, y: 0.88, w: 10, h: 0.04, fill: { color: C.gold }
    });
    ...
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0.5, y: 1.1, w: 0.08, h: 1.0, fill: { color: C.gold }
    });
    ...
    for (const c of cards) {
      ...
      slide.addShape(pres.shapes.RECTANGLE, {
        x: c.x, y: c.y, w: 0.06, h: 1.45, fill: { color: C.gold }
      });
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    for (let i = 0; i < pillars.length; i++) {
      ...
      slide.addText(p.points.map((pt, idx) => ({
        ...
        x: x + 0.15, y: 4.2, w: 2.6, h: 0.7,
        fontSize: 10, fontFace: "Arial", color: C.text, margin: 0
      });
      
      // Badge
      slide.addShape(pres.shapes.RECTANGLE, {
        x: x + 0.35, y: 4.9, w: 2.2, h: 0.3, fill: { color: p.color, transparency: 80 }
      });
      slide.addText(p.badge, {
        x: x + 0.35, y: 4.9, w: 2.2, h: 0.3,
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    for (let i = 0; i < pillars4.length; i++) {
      ...
      slide.addShape(pres.shapes.RECTANGLE, {
        x: 0.5, y: y, w: 0.06, h: 0.82, fill: { color: C.gold }
      });
      
      // Number badge
      slide.addShape(pres.shapes.OVAL, {
        x: 0.72, y: y + 0.16, w: 0.5, h: 0.5, fill: { color: C.gold }
      });
      slide.addText(p.num, {
        x: 0.72, y: y + 0.16, w: 0.5, h: 0.5,
        fontSize: 16, fontFace: "Arial", color: C.midnight, bold: true, align: "center", valign: "middle", margin: 0
      });
      ...
      slide.addText("目标用户：" + p.users, {
        ...
        fontSize: 11, fontFace: "Arial", color: C.goldLight, margin: 0
      });
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    for (let i = 0; i < regions.length; i++) {
      ...
      for (let j = 0; j < rows.length; j++) {
        ...
        // Gold dot
        slide.addShape(pres.shapes.OVAL, {
          x: x + 0.15, y: y + 0.08, w: 0.12, h: 0.12, fill: { color: C.gold }
        });
        slide.addText(row.label, {
          x: x + 0.35, y: y, w: 0.9, h: 0.25,
          fontSize: 9, fontFace: "Arial", color: C.gold, bold: true, margin: 0
        });
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    for (let i = 0; i < headers.length; i++) {
      slide.addText(headers[i], {
        ...
        fontSize: 11, fontFace: "Arial", color: C.gold, bold: true, align: "center", valign: "middle", margin: 0
      });
    }
    ...
    for (let i = 0; i < materials.length; i++) {
      ...
      for (let j = 0; j < vals.length; j++) {
        slide.addText(vals[j], {
          ...
          fontSize: 10, fontFace: "Arial", color: j === 0 ? C.goldLight : C.text, 
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    for (let i = 0; i < kolHeaders.length; i++) {
      slide.addText(kolHeaders[i], {
        ...
        fontSize: 10, fontFace: "Arial", color: C.gold, bold: true, align: "center", valign: "middle", margin: 0
      });
    }
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    // Central hub
    slide.addShape(pres.shapes.OVAL, {
      x: 3.8, y: 1.6, w: 2.4, h: 2.4, fill: { color: C.gold }
    });
    slide.addShape(pres.shapes.OVAL, {
    ...
    slide.addText("全部目标市场\n7月同步上线", {
      ...
      fontSize: 14, fontFace: "Arial", color: C.gold, bold: true, align: "center", valign: "middle", margin: 0
    });
    ...
    slide.addText("亚洲+拉美+美国同步发行，非分批上线", {
      ...
      fontSize: 12, fontFace: "Arial", color: C.gold, bold: true, align: "center", valign: "middle", margin: 0
    });
  }
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    for (let i = 0; i < budgets.length; i++) {
      ...
      slide.addText(b.pct, {
        ...
        fontSize: 11, fontFace: "Arial", color: C.midnight, bold: true, align: "center", valign: "middle", margin: 0
      });
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    for (let i = 0; i < priorities.length; i++) {
      ...
      slide.addText("→", {
        ...
        fontSize: 18, fontFace: "Arial", color: C.gold, margin: 0
      });
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    slide.addText("下一步", {
      ...
      fontSize: 40, fontFace: "Arial Black", color: C.gold, align: "center", charSpacing: 8, margin: 0
    });
    ...
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 1.8, y: 4.6, w: 0.08, h: 0.6, fill: { color: C.accent }
    });
    slide.addText("🎯 目标：7月爆发期全力冲击下载 + 榜单", {
      x: 2.0, y: 4.6, w: 6.1, h: 0.6,
      fontSize: 16, fontFace: "Arial", color: C.accent, bold: true, valign: "middle", margin: 0
    });
  ...
  // ── Save ──────────────────────────────────────────────────────────────────
  await pres.writeFile({ fileName: "/Users/shadow/.openclaw/workspace/GoldAndGlory_七月发行策略_v4.pptx" });
  console.log("✅ Done! Saved to GoldAndGlory_七月发行策略_v4.pptx");
}
