...
async function build() {
  ...
  pres.title = "Gold and Glory - 发行策略";
  ...
  slide1.addText("2026年4月  |  Confidential — Internal Use Only", {
  ...
  slide2.background = { color: C.darkBg };
  ...
  slide3.addText("7月大版本核心内容", {
    x: 0.5, y: 0.2, w: 9, h: 0.6,
    fontSize: 26, fontFace: "Arial", color: C.text, bold: true, margin: 0
  });
  slide3.addText("三大营销素材支柱（按重要度排序）", {
  ...
  slide3.addText("三个内容支柱构成7月营销的核心素材库，所有广告素材、KOL合作均围绕这三个展开。", {
  ...
  slide4.background = { color: C.darkBg };
  ...
  slide4.addText("核心USP：进去抢，活着出来就是你的（Extraction Shooter独家机制）", {
  ...
  for (let i = 0; i < pillars4.length; i++) {
    ...
    slide4.addText("切入角度：" + p.angle, {
      x: 1.4, y: y + 0.42, w: 2.7, h: 0.32,
      fontSize: 9, fontFace: "Arial", color: C.text, margin: 0
    });
    slide4.addText("目标用户：" + p.users, {
      x: 4.2, y: y + 0.1, w: 2.4, h: 0.32,
      fontSize: 10, fontFace: "Arial", color: C.purpleLight, margin: 0
    });
    slide4.addText("内容：" + p.content, {
      x: 4.2, y: y + 0.42, w: 5.1, h: 0.32,
      fontSize: 9, fontFace: "Arial", color: C.textMuted, margin: 0
    });
  }

  await pres.writeFile({ fileName: "/Users/shadow/.openclaw/workspace/beautified_ppt.pptx" });
  console.log("✅ Done! beautified_ppt.pptx");
}
