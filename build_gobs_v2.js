...
// Light Professional Style - Enhanced
const C = {
  blue: "3B82F6",
  blueLight: "60A5FA",
  blueDark: "1D4ED8",
...
async function build() {
  ...
  // ===== 中心：Gobs 平台 =====
  // Outer glow
  s2.addShape(pres.shapes.OVAL, { x: 3.8, y: 1.8, w: 2.4, h: 2.4, fill: { color: C.blue, transparency: 90 } });
  s2.addShape(pres.shapes.OVAL, { x: 4.0, y: 2.0, w: 2.0, h: 2.0, fill: { color: C.blue, transparency: 85 } });
  // Center circle
  s2.addShape(pres.shapes.OVAL, { x: 4.2, y: 2.2, w: 1.6, h: 1.6, fill: { color: C.blue } });
  s2.addText("Gobs\n平台", { x: 4.2, y: 2.2, w: 1.6, h: 1.6, fontSize: 18, fontFace: "Arial", color: C.white, bold: true, align: "center", valign: "middle", margin: 0 });

  // ===== 左：数据洞察 =====
  // Card
  s2.addShape(pres.shapes.RECTANGLE, { x: 0.4, y: 1.3, w: 2.8, h: 3.5, fill: { color: C.white }, line: { color: C.blue, width: 2 } });
  s2.addShape(pres.shapes.RECTANGLE, { x: 0.4, y: 1.3, w: 2.8, h: 0.7, fill: { color: C.blue } });
  s2.addText("📊 数据洞察", { x: 0.4, y: 1.3, w: 2.8, h: 0.7, fontSize: 14, fontFace: "Arial", color: C.white, bold: true, align: "center", valign: "middle", margin: 0 });
  
  // Sub-items with icons
  const dataItems = [
    { icon: "🎮", text: "Ingame", sub: "接数据后台" },
    { icon: "🌐", text: "游戏声量", sub: "接Crawly" },
    { icon: "💬", text: "游戏舆情", sub: "分渠道监控" },
    { icon: "📈", text: "OM获客", sub: "（暂无）" },
  ];
  for (let i = 0; i < dataItems.length; i++) {
    const item = dataItems[i];
    const y = 2.15 + i * 0.7;
    s2.addShape(pres.shapes.OVAL, { x: 0.6, y: y, w: 0.4, h: 0.4, fill: { color: C.blue, transparency: 80 } });
    s2.addText(item.icon, { x: 0.6, y: y, w: 0.4, h: 0.4, fontSize: 14, align: "center", valign: "middle" });
    s2.addText(item.text, { x: 1.1, y: y - 0.05, w: 1.8, h: 0.3, fontSize: 12, fontFace: "Arial", color: C.gray, bold: true, margin: 0 });
    s2.addText(item.sub, { x: 1.1, y: y + 0.22, w: 1.8, h: 0.25, fontSize: 9, fontFace: "Arial", color: C.grayLight, margin: 0 });
  }

  // ===== 右：Action =====
  s2.addShape(pres.shapes.RECTANGLE, { x: 6.8, y: 1.3, w: 2.8, h: 3.5, fill: { color: C.white }, line: { color: C.green, width: 2 } });
  s2.addShape(pres.shapes.RECTANGLE, { x: 6.8, y: 1.3, w: 2.8, h: 0.7, fill: { color: C.green } });
  s2.addText("⚡ Action", { x: 6.8, y: 1.3, w: 2.8, h: 0.7, fontSize: 14, fontFace: "Arial", color: C.white, bold: true, align: "center", valign: "middle", margin: 0 });
  
  const actionItems = [
    { icon: "🎬", text: "素材生成", sub: "与分发", highlight: true },
    { icon: "🤖", text: "水军运营", sub: "多账号分发", highlight: true },
    { icon: "🛠", text: "Live Ops", sub: "Bug反馈→Jira" },
    { icon: "📋", text: "产品建议", sub: "转需求" },
  ];
  for (let i = 0; i < actionItems.length; i++) {
    const item = actionItems[i];
    const y = 2.15 + i * 0.7;
    const bgColor = item.highlight ? C.green : C.grayLighter;
    const textColor = item.highlight ? C.white : C.gray;
    s2.addShape(pres.shapes.ROUNDED_RECTANGLE, { x: 7.0, y: y - 0.05, w: 2.4, h: 0.6, fill: { color: bgColor }, rectRadius: 0.05 });
    s2.addText(item.icon + " " + item.text, { x: 7.1, y: y - 0.05, w: 2.2, h: 0.35, fontSize: 12, fontFace: "Arial", color: item.highlight ? C.white : C.gray, bold: true, valign: "middle", margin: 0 });
    s2.addText(item.sub, { x: 7.1, y: y + 0.28, w: 2.2, h: 0.25, fontSize: 9, fontFace: "Arial", color: item.highlight ? C.white : C.grayLight, valign: "top", margin: 0 });
  }

  // ===== 上：Campaign =====
  s2.addShape(pres.shapes.RECTANGLE, { x: 3.3, y: 0.95, w: 3.4, h: 0.8, fill: { color: C.purple } });
  s2.addText("📋 Campaign", { x: 3.3, y: 0.95, w: 3.4, h: 0.45, fontSize: 14, fontFace: "Arial", color: C.white, bold: true, align: "center", valign: "middle", margin: 0 });
  s2.addText("主动式 Agent · 发行方案生成", { x: 3.3, y: 1.38, w: 3.4, h: 0.35, fontSize: 10, fontFace: "Arial", color: C.purpleLight, align: "center", margin: 0 });

  // ===== 连接线 =====
  // Data → Center
  s2.addShape(pres.shapes.LINE, { x: 3.2, y: 3.05, w: 1.0, h: 0, line: { color: C.blue, width: 2, dashType: "dash" } });
  // Campaign → Center
  s2.addShape(pres.shapes.LINE, { x: 5.0, y: 1.75, w: 0, h: 0.45, line: { color: C.purple, width: 2 } });
  // Center → Action
  s2.addShape(pres.shapes.LINE, { x: 5.8, y: 3.05, w: 1.0, h: 0, line: { color: C.green, width: 2, dashType: "dash" } });

  // ===== 底部：知识库 =====
  s2.addShape(pres.shapes.RECTANGLE, { x: 2.8, y: 4.5, w: 4.4, h: 0.9, fill: { color: C.yellow, transparency: 90 }, line: { color: C.yellow, width: 1 } });
  s2.addText("📚 知识库", { x: 2.8, y: 4.55, w: 4.4, h: 0.4, fontSize: 12, fontFace: "Arial", color: C.yellow, bold: true, align: "center", margin: 0 });
  s2.addText("基于 Andrej Karpathy 工作流 · 进来需先绑定游戏", { x: 2.8, y: 4.92, w: 4.4, h: 0.35, fontSize: 10, fontFace: "Arial", color: C.gray, align: "center", margin: 0 });

  // ===== 图例 =====
  s2.addText("——→ 数据流", { x: 0.4, y: 5.2, w: 1.5, h: 0.3, fontSize: 9, fontFace: "Arial", color: C.grayLight });
  s2.addShape(pres.shapes.LINE, { x: 1.9, y: 5.35, w: 0.5, h: 0, line: { color: C.grayLight, width: 1, dashType: "dash" } });
  s2.addText("──→ 执行流", { x: 2.5, y: 5.2, w: 1.5, h: 0.3, fontSize: 9, fontFace: "Arial", color: C.grayLight });
  s2.addShape(pres.shapes.LINE, { x: 3.9, y: 5.35, w: 0.5, h: 0, line: { color: C.grayLight, width: 1 } });
  ...
  s4.addText("Day 0-30：当前阶段（建设中）", { x: 0.5, y: 0.2, w: 7, h: 0.6, fontSize: 24, fontFace: "Arial", color: C.gray, bold: true, margin: 0 });
  ...
  await pres.writeFile({ fileName: "/Users/shadow/.openclaw/workspace/Gobs_Pipeline_v2.pptx" });
  console.log("✅ Done! Gobs_Pipeline_v2.pptx");
}
