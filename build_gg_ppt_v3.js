...
// ─── Color Palette: Dark Dungeon Gold Theme ─────────────────────────────────────
const C = {
  gold: "D4A853",        
  goldLight: "F5D78E",   
  goldDark: "B8860B",   
  midnight: "0A0E17",     // Deepest dungeon black
  darkBg: "0D1117",       
  cardBg: "161B22",       
  cardElevated: "1C2128", 
  slate: "21262D",       
  text: "F0F6FC",        
  textMuted: "8B949E",   
  accent: "FF6B35",       // Fire/orange accent
  success: "3FB950",     
  warning: "D29922",     
  danger: "F85149",      
  purple: "A371F7",      
...
// ─── Shadow Factories ───────────────────────────────────────────────────────────
const makeShadow = () => ({
  type: "outer", blur: 10, offset: 4, angle: 135, color: "000000", opacity: 0.4
});
...
const makeGlow = (color) => ({
  type: "outer", blur: 15, offset: 0, angle: 0, color: color, opacity: 0.2
});
...
async function buildPresentation() {
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // SLIDE 1: Title Slide - Epic Dark Dungeon Style
  // ══════════════════════════════════════════════════════════════════════════
  {
    let slide = pres.addSlide();
    slide.background = { color: C.midnight };
    
    // Top gradient bar (gold fade)
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0, y: 0, w: 10, h: 0.12, fill: { color: C.gold }
    });
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0, y: 0.12, w: 10, h: 0.04, fill: { color: C.goldLight, transparency: 60 }
    });
    
    // Left decorative vertical bar with glow effect
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0.4, y: 1.2, w: 0.06, h: 2.8, fill: { color: C.gold }, shadow: makeGlow(C.gold)
    });
    
    // Decorative diamond stack (left side)
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0.55, y: 1.5, w: 0.35, h: 0.35, fill: { color: C.gold }, rotate: 45
    });
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0.65, y: 1.85, w: 0.25, h: 0.25, fill: { color: C.goldLight }, rotate: 45
    });
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0.72, y: 2.15, w: 0.18, h: 0.18, fill: { color: C.goldDark }, rotate: 45
    });
    
    // Main title - Glowing gold text
    slide.addText("GOLD AND GLORY", {
      x: 1.2, y: 1.5, w: 8, h: 1.0,
      fontSize: 56, fontFace: "Arial Black", color: C.gold,
      bold: true, charSpacing: 6, shadow: makeShadow()
    });
    
    // Subtitle
    slide.addText("7月大版本完整发行策略", {
      x: 1.2, y: 2.5, w: 8, h: 0.7,
      fontSize: 30, fontFace: "Arial", color: C.text, bold: true
    });
    
    // Tagline with icons
    slide.addText("Arena  ·  PVE  ·  3C升级  ·  UA买量  ·  本地化", {
      x: 1.2, y: 3.3, w: 8, h: 0.5,
      fontSize: 16, fontFace: "Arial", color: C.goldLight
    });
    
    // Divider with diamond
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 1.2, y: 4.0, w: 2.5, h: 0.02, fill: { color: C.gold, transparency: 50 }
    });
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 3.8, y: 3.95, w: 0.12, h: 0.12, fill: { color: C.gold }, rotate: 45
    });
    
    // Date & confidential
    slide.addText("2026年4月  |  Confidential — Internal Use Only", {
      x: 1.2, y: 4.2, w: 8, h: 0.4,
      fontSize: 12, fontFace: "Arial", color: C.textMuted
    });
    
    // Bottom gold bar
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0, y: 5.505, w: 10, h: 0.12, fill: { color: C.gold }
    });
  }
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0, y: 0, w: 10, h: 0.85, fill: { color: C.cardBg }
    });
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0, y: 0.83, w: 10, h: 0.04, fill: { color: C.gold }
    });
    
    // Diamond icon in header
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0.45, y: 0.22, w: 0.4, h: 0.4, fill: { color: C.gold }, rotate: 45
    });
    slide.addText("产品核心定位", {
      x: 1.1, y: 0.2, w: 8, h: 0.5,
      fontSize: 26, fontFace: "Arial", color: C.text, bold: true
    });
    
    // USP Quote box - highlighted with gold left border
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0.5, y: 1.1, w: 9, h: 1.1, fill: { color: C.cardBg }, shadow: makeCardShadow()
    });
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0.5, y: 1.1, w: 0.08, h: 1.1, fill: { color: C.gold }
    });
    // Quote marks
    slide.addText("\u201C", {
      x: 0.7, y: 1.05, w: 0.5, h: 0.6,
      fontSize: 48, fontFace: "Georgia", color: C.gold, bold: true
    });
    slide.addText("进去抢，活着出来就是你的", {
      x: 1.2, y: 1.2, w: 8, h: 0.55,
      fontSize: 24, fontFace: "Georgia", color: C.gold, italic: true, align: "center"
    });
    slide.addText("— G&G 核心 USP，Mobile Extraction Shooter 独家机制", {
      x: 1.2, y: 1.75, w: 8, h: 0.35,
      fontSize: 11, fontFace: "Arial", color: C.textMuted, align: "center"
    });
    
    // 4 Product cards - 2x2 grid
    const cards = [
      { title: "产品定位", content: "中世纪地牢 Extraction RPG\n5–15分钟快节奏单局\n赛季制 + 英雄收集", x: 0.5, y: 2.4, icon: "⚔" },
      { title: "核心特色", content: "Mobile 端独家 Extraction Shooter\n风险回报机制\n死后全失的刺激感", x: 5.05, y: 2.4, icon: "🛡" },
      { title: "当前状态", content: "Google Play 评分：3.7 ⚠\n已测试：墨西哥、哥伦比亚\n服务器：4–6月停服维护", x: 0.5, y: 4.0, icon: "⚠" },
      { title: "7月目标", content: "7月大版本发布在即\n⚠ 3.7分是市场门槛\n目标：7月前改善至 3.8–4.0", x: 5.05, y: 4.0, icon: "🎯" },
    ];
    
    for (const c of cards) {
      // Card background
      slide.addShape(pres.shapes.RECTANGLE, {
        x: c.x, y: c.y, w: 4.45, h: 1.4, fill: { color: C.cardBg }, shadow: makeCardShadow()
      });
      // Left gold accent
      slide.addShape(pres.shapes.RECTANGLE, {
        x: c.x, y: c.y, w: 0.06, h: 1.4, fill: { color: C.gold }
      });
      // Icon circle
      slide.addShape(pres.shapes.OVAL, {
        x: c.x + 0.2, y: c.y + 0.15, w: 0.5, h: 0.5, fill: { color: C.slate }
      });
      ...
      slide.addText(c.title, {
        x: c.x + 0.8, y: c.y + 0.18, w: 3.4, h: 0.4,
        fontSize: 15, fontFace: "Arial", color: C.gold, bold: true
      });
      // Content
      slide.addText(c.content, {
        x: c.x + 0.2, y: c.y + 0.65, w: 4, h: 0.7,
        fontSize: 11, fontFace: "Arial", color: C.text, lineSpacingMultiple: 1.25
      });
    }
  }
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    const pillars = [
      ...
      },
      {
        ...
        badge: "核心玩法质变", color: C.teal, symbol: "🎮"
      }
    ];
    ...
    for (let i = 0; i < pillars.length; i++) {
      ...
      // Card
      slide.addShape(pres.shapes.RECTANGLE, {
        x: x, y: 1.45, w: 2.9, h: 3.7, fill: { color: C.cardBg }, shadow: makeCardShadow()
      });
      
      // Top color accent bar
      slide.addShape(pres.shapes.RECTANGLE, {
        x: x, y: 1.45, w: 2.9, h: 0.08, fill: { color: p.color }
      });
      
      // Rank badge
      slide.addShape(pres.shapes.ROUNDED_RECTANGLE, {
        x: x + 0.15, y: 1.6, w: 0.7, h: 0.45, fill: { color: p.color }, rectRadius: 0.05
      });
      slide.addText(p.rank, {
        x: x + 0.15, y: 1.6, w: 0.7, h: 0.45,
        fontSize: 18, fontFace: "Arial Black", color: C.midnight, bold: true, align: "center", valign: "middle"
      });
      
      // Stars
      slide.addText(p.stars, {
        x: x + 0.9, y: 1.65, w: 1.8, h: 0.35,
        fontSize: 11, color: p.color
      });
      
      // Symbol icon
      slide.addShape(pres.shapes.OVAL, {
        x: x + 0.95, y: 2.1, w: 1.0, h: 1.0, fill: { color: C.slate }
      });
      slide.addText(p.symbol, {
        x: x + 0.95, y: 2.1, w: 1.0, h: 1.0,
      ...
      slide.addText(p.title, {
        ...
        fontSize: 15, fontFace: "Arial", color: C.text, bold: true, align: "center"
      });
      
      // Tags
      slide.addText(p.tags, {
        x: x + 0.1, y: 3.6, w: 2.7, h: 0.5,
      ...
      slide.addText(p.points.map((pt, idx) => ({
        ...
        x: x + 0.15, y: 4.1, w: 2.6, h: 0.65,
        fontSize: 9, fontFace: "Arial", color: C.text
      });
      
      // Badge
      slide.addShape(pres.shapes.ROUNDED_RECTANGLE, {
        x: x + 0.35, y: 4.8, w: 2.2, h: 0.3, fill: { color: p.color, transparency: 80 }, rectRadius: 0.03
      });
      slide.addText(p.badge, {
        x: x + 0.35, y: 4.8, w: 2.2, h: 0.3,
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    // Core USP highlight box
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0.5, y: 1.0, w: 9, h: 0.75, fill: { color: C.cardBg }, shadow: makeCardShadow()
    });
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0.5, y: 1.0, w: 0.08, h: 0.75, fill: { color: C.gold }
    });
    slide.addText("核心USP", {
      x: 0.75, y: 1.05, w: 1.2, h: 0.3,
      fontSize: 10, fontFace: "Arial", color: C.gold, bold: true
    });
    slide.addText("进去抢，活着出来就是你的（Extraction Shooter独家机制）", {
      x: 0.75, y: 1.32, w: 8.5, h: 0.35,
    ...
    for (let i = 0; i < pillars4.length; i++) {
      ...
      // Row card
      slide.addShape(pres.shapes.RECTANGLE, {
        x: 0.5, y: y, w: 9, h: 0.8, fill: { color: C.cardBg }, shadow: makeCardShadow()
      });
      slide.addShape(pres.shapes.RECTANGLE, {
        x: 0.5, y: y, w: 0.06, h: 0.8, fill: { color: C.gold }
      });
      
      // Number badge
      slide.addShape(pres.shapes.OVAL, {
        x: 0.72, y: y + 0.15, w: 0.5, h: 0.5, fill: { color: C.gold }
      });
      slide.addText(p.num, {
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    for (let i = 0; i < regions.length; i++) {
      ...
      // Card
      slide.addShape(pres.shapes.RECTANGLE, {
        x: x, y: 1.4, w: 2.9, h: 3.85, fill: { color: C.cardBg }, shadow: makeCardShadow()
      });
      // Header section
      slide.addShape(pres.shapes.RECTANGLE, {
        x: x, y: 1.4, w: 2.9, h: 0.6, fill: { color: C.cardElevated }
      });
      
      // Flag & Name
      slide.addText(r.flag + " " + r.name, {
        x: x + 0.15, y: 1.45, w: 2.6, h: 0.5,
        fontSize: 13, fontFace: "Arial", color: C.text, bold: true, valign: "middle"
      });
      ...
      for (let j = 0; j < rows.length; j++) {
        ...
        // Label with gold dot
        slide.addShape(pres.shapes.OVAL, {
          x: x + 0.12, y: y + 0.06, w: 0.12, h: 0.12, fill: { color: C.gold }
        });
        slide.addText(row.label, {
          x: x + 0.3, y: y, w: 0.8, h: 0.22,
          fontSize: 8, fontFace: "Arial", color: C.gold, bold: true
        });
        slide.addText(row.value, {
          x: x + 0.12, y: y + 0.22, w: 2.65, h: 0.35,
          fontSize: 9, fontFace: "Arial", color: C.text
        });
      }
    }
    ...
    // Warning note
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0.5, y: 5.35, w: 9, h: 0.22, fill: { color: C.warning, transparency: 85 }
    });
    slide.addText("※ 非正式上线（测试阶段），预算有限。OM为主，KOL/KOC按区域单独评估", {
      x: 0.6, y: 5.35, w: 8.8, h: 0.22,
      fontSize: 9, fontFace: "Arial", color: C.warning, valign: "middle"
    });
  }
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0.5, y: 1.0, w: 9, h: 0.45, fill: { color: C.cardElevated }
    });
    for (let i = 0; i < headers.length; i++) {
      slide.addShape(pres.shapes.RECTANGLE, {
        x: colX[i], y: 1.0, w: colW[i], h: 0.45, fill: { color: C.slate, transparency: 50 }
      });
      slide.addText(headers[i], {
        x: colX[i], y: 1.0, w: colW[i], h: 0.45,
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0.5, y: 1.28, w: 9, h: 0.45, fill: { color: C.cardElevated }
    });
    for (let i = 0; i < kolHeaders.length; i++) {
      slide.addShape(pres.shapes.RECTANGLE, {
        x: kolColX[i], y: 1.28, w: kolColW[i], h: 0.45, fill: { color: C.slate, transparency: 50 }
      });
      slide.addText(kolHeaders[i], {
        x: kolColX[i], y: 1.28, w: kolColW[i], h: 0.45,
    ...
    for (let i = 0; i < kolTiers.length; i++) {
      ...
      slide.addShape(pres.shapes.RECTANGLE, {
        x: 0.5, y: y, w: 9, h: 0.7, fill: { color: bgColor }
      });
      slide.addShape(pres.shapes.RECTANGLE, {
        x: 0.5, y: y, w: 0.05, h: 0.7, fill: { color: C.gold }
      });
      
      const vals = [k.name, k.fans, k.market, k.platform, k.cpm, k.pillar, k.angle];
      for (let j = 0; j < vals.length; j++) {
        slide.addText(vals[j], {
          x: kolColX[j], y: y, w: kolColW[j], h: 0.7,
          fontSize: j === 0 ? 11 : 9, 
          fontFace: "Arial", 
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    // Central hub circle with glow
    slide.addShape(pres.shapes.OVAL, {
      x: 3.8, y: 1.6, w: 2.4, h: 2.4, fill: { color: C.gold }, shadow: makeShadow()
    });
    slide.addShape(pres.shapes.OVAL, {
      x: 3.95, y: 1.75, w: 2.1, h: 2.1, fill: { color: C.cardBg }
    });
    slide.addText("全部目标市场\n7月同步上线", {
      x: 3.8, y: 2.15, w: 2.4, h: 1.3,
      fontSize: 14, fontFace: "Arial", color: C.gold, bold: true, align: "center", valign: "middle"
    });
    
    // Country nodes around center
    const countries = [
      { flag: "🇮🇩", name: "印尼", x: 1.3, y: 1.4 },
      { flag: "🇻🇳", name: "越南", x: 6.8, y: 1.4 },
      { flag: "🇲🇽", name: "墨西哥", x: 0.6, y: 2.9 },
      { flag: "🇧🇷", name: "巴西", x: 7.6, y: 2.9 },
      { flag: "🇨🇴", name: "哥伦比亚", x: 1.0, y: 4.3 },
      { flag: "🇦🇷", name: "阿根廷", x: 4.0, y: 4.8 },
      { flag: "🇨🇱", name: "智利", x: 5.9, y: 4.3 },
      { flag: "🇺🇸", name: "美国", x: 8.1, y: 3.8 },
    ];
    
    for (const c of countries) {
      // Connector line (simplified - just card)
      slide.addShape(pres.shapes.RECTANGLE, {
        x: c.x, y: c.y, w: 1.35, h: 0.65, fill: { color: C.cardBg }, shadow: makeCardShadow()
      });
      slide.addShape(pres.shapes.RECTANGLE, {
        x: c.x, y: c.y, w: 0.05, h: 0.65, fill: { color: C.gold }
      });
      slide.addText(c.flag + " " + c.name, {
        x: c.x + 0.1, y: c.y, w: 1.15, h: 0.65,
        fontSize: 11, fontFace: "Arial", color: C.text, align: "center", valign: "middle"
      });
    }
    
    // Bottom highlight
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 2.5, y: 5.35, w: 5, h: 0.25, fill: { color: C.gold, transparency: 85 }
    });
    slide.addText("亚洲+拉美+美国同步发行，非分批上线", {
      x: 2.5, y: 5.35, w: 5, h: 0.25,
      fontSize: 11, fontFace: "Arial", color: C.gold, bold: true, align: "center", valign: "middle"
    });
  }
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    // Timeline bar
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0.8, y: 2.35, w: 8.4, h: 0.1, fill: { color: C.slate }
    });
    ...
    for (let i = 0; i < phases.length; i++) {
      ...
      // Node circle
      slide.addShape(pres.shapes.OVAL, {
        x: x + 0.55, y: 2.15, w: 0.5, h: 0.5, fill: { color: p.color }, shadow: makeCardShadow()
      });
      
      // Month
      slide.addText(p.month, {
        x: x, y: 1.65, w: 1.6, h: 0.4,
    ...
    // Detail cards
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0.5, y: 3.5, w: 4.4, h: 1.95, fill: { color: C.cardBg }, shadow: makeCardShadow()
    });
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 0.5, y: 3.5, w: 0.06, h: 1.95, fill: { color: C.gold }
    });
    slide.addText("准备期 3月", {
      x: 0.7, y: 3.55, w: 4, h: 0.35,
    ...
    slide.addText([
      ...
      x: 0.7, y: 3.9, w: 4, h: 1.4,
      fontSize: 10, fontFace: "Arial", color: C.text, lineSpacingMultiple: 1.3
    });
    
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 5.1, y: 3.5, w: 4.4, h: 1.95, fill: { color: C.cardBg }, shadow: makeCardShadow()
    });
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 5.1, y: 3.5, w: 0.06, h: 1.95, fill: { color: C.accent }
    });
    slide.addText("爆发期 7月", {
      x: 5.3, y: 3.55, w: 4, h: 0.35,
      fontSize: 12, fontFace: "Arial", color: C.accent, bold: true
    });
    slide.addText([
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    // Budget bars with visual impact
    const budgets = [
    ...
    for (let i = 0; i < budgets.length; i++) {
      ...
      // Bar background
      slide.addShape(pres.shapes.RECTANGLE, {
        x: 2.1, y: y + 0.12, w: 5.5, h: 0.32, fill: { color: C.slate }
      });
      
      // Bar fill with shadow
      slide.addShape(pres.shapes.RECTANGLE, {
        x: 2.1, y: y + 0.12, w: barW, h: 0.32, fill: { color: b.color }, shadow: makeCardShadow()
      });
      
      // Percentage circle
      slide.addShape(pres.shapes.OVAL, {
        x: 7.7, y: y + 0.05, w: 0.45, h: 0.45, fill: { color: b.color }
      });
      slide.addText(b.pct, {
        x: 7.7, y: y + 0.05, w: 0.45, h: 0.45,
        fontSize: 11, fontFace: "Arial", color: C.midnight, bold: true, align: "center", valign: "middle"
      });
      
      // Description
      slide.addText(b.desc, {
        x: 0.5, y: y + 0.42, w: 8, h: 0.35,
        fontSize: 9, fontFace: "Arial", color: C.textMuted
      });
    }
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    for (let i = 0; i < priorities.length; i++) {
      ...
      // Card
      slide.addShape(pres.shapes.RECTANGLE, {
        x: 0.5, y: y, w: 9, h: 1.25, fill: { color: C.cardBg }, shadow: makeCardShadow()
      });
      slide.addShape(pres.shapes.RECTANGLE, {
        x: 0.5, y: y, w: 0.08, h: 1.25, fill: { color: p.color }
      });
      
      // Label badge
      slide.addShape(pres.shapes.ROUNDED_RECTANGLE, {
        x: 0.75, y: y + 0.12, w: 1.5, h: 0.32, fill: { color: p.color, transparency: 75 }, rectRadius: 0.03
      });
      slide.addText(p.label, {
        x: 0.75, y: y + 0.12, w: 1.5, h: 0.32,
        fontSize: 10, fontFace: "Arial", color: p.color, bold: true, align: "center", valign: "middle"
      });
      
      // Arrow
      slide.addText("→", {
        x: 2.35, y: y + 0.05, w: 0.3, h: 0.4,
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    // Decorative diamonds
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 4.55, y: 0.8, w: 0.5, h: 0.5, fill: { color: C.gold }, rotate: 45
    });
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 4.25, y: 1.05, w: 0.3, h: 0.3, fill: { color: C.goldLight }, rotate: 45
    });
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 5.1, y: 1.05, w: 0.3, h: 0.3, fill: { color: C.goldLight }, rotate: 45
    });
    
    // Title
    slide.addText("下一步", {
      x: 0.5, y: 1.7, w: 9, h: 0.8,
      fontSize: 40, fontFace: "Arial Black", color: C.gold, align: "center", charSpacing: 8
    });
    ...
    for (let i = 0; i < nextSteps.length; i++) {
      ...
      // Number circle
      slide.addShape(pres.shapes.OVAL, {
        x: 2.0, y: y, w: 0.45, h: 0.45, fill: { color: C.gold }
      });
      slide.addText(String(i + 1), {
        x: 2.0, y: y, w: 0.45, h: 0.45,
        fontSize: 15, fontFace: "Arial", color: C.midnight, bold: true, align: "center", valign: "middle"
      });
      
      // Text
      slide.addText(nextSteps[i], {
        x: 2.65, y: y, w: 5.5, h: 0.45,
    ...
    // Goal box
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 1.8, y: 4.9, w: 6.4, h: 0.55, fill: { color: C.cardBg }, shadow: makeCardShadow()
    });
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 1.8, y: 4.9, w: 0.08, h: 0.55, fill: { color: C.accent }
    });
    slide.addText("🎯 目标：7月爆发期全力冲击下载 + 榜单", {
      x: 2.0, y: 4.9, w: 6.1, h: 0.55,
      fontSize: 15, fontFace: "Arial", color: C.accent, bold: true, valign: "middle"
    });
  ...
  // ── Save ──────────────────────────────────────────────────────────────────
  await pres.writeFile({ fileName: "/Users/shadow/.openclaw/workspace/GoldAndGlory_七月发行策略_v3.pptx" });
  console.log("✅ Done! Saved to GoldAndGlory_七月发行策略_v3.pptx");
}
